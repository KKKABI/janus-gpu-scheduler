"""Benchmark ONE model, DRT strategy (v2.1 worktree)"""
...
for alpha in [0.9, 0.8, 0.5, 0.2]:
    print(f"  DRT α={alpha} ...", file=sys.stderr)
    ...
    row = f"{model_name},DRT,{alpha},{np.median(times):.4f},{np.min(times):.4f},{np.max(times):.4f}"
    results.append(row)
...
print(f"Done {model_name}: {len(results)} DRT rows", file=sys.stderr)
