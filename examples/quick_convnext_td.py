"""Quick ConvNeXt Time-domain: alpha 0.9 and 0.8 only"""
import os, sys
...
for alpha in [0.9, 0.8]:
    ...
    with redirect_stdout(f):
        runner = GraphCapturer.capturer(
            ...
            alpha=alpha, selection_mode='cosine', time_domain=True
        )
    cands = [s.get(f'a={alpha}', 0) for s in _CANDIDATE_STATS]
    total = len(cands)
    avg = sum(cands) / total if total else 0
    single = sum(1 for c in cands if c <= 1)
    print(f"α={alpha}: calls={total} avg_cand={avg:.1f} only1={single}/{total} ({100*single/total:.0f}%) min={min(cands)} max={max(cands)}")
    _CANDIDATE_STATS.clear()
    ...
    print(f"  latency: median={np.median(times):.3f} min={np.min(times):.3f} max={np.max(times):.3f}")
