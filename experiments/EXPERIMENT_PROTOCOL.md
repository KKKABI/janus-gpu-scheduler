...
- **Baseline 的唯一含义**：`GraphCapturer.capturer(inputs, model, copy_outputs=False)`，不传 `alpha`、`selection_mode` 或 `time_domain`。Baseline 在每个模型上只运行一次，不复制到不同 α 行。
...
模型构造、权重来源和输入形状由 `experiments/repro_config.json` 固定。外部权重文件必须通过 `experiments/model_asset_manifest.sha256` 校验，profile 文件必须通过 `experiments/profile_manifest.sha256` 校验；一轮矩阵执行期间不得重新生成或替换这些文件。
...
2. 创建 4 MiB CUDA int8 cache buffer；每轮执行前调用 `zero_()`，与现有脚本保持一致。
3. 预热 50 次，随后 `torch.cuda.synchronize()`。
4. 用同一 CUDA stream 上的 CUDA Events 采集 100 个单次延迟样本；每个样本结束后同步。
5. 保存全部 100 个原始样本，并记录实验后的 GPU 遥测。
6. 每次进程内统计 median/min/max；跨 5 个进程的主指标是“5 个进程内 median 的 median”，同时报告 mean、样本标准差和 MAD。min/max 仅作诊断，不作为优劣结论。
