    ...
    def __init__(self, module, targets):
        ...
        class _Recorder(torch.fx.Interpreter):
            ...
            def run_node(inner, node):
                ...
                # In-place FX nodes (notably YOLO SiLU_) mutate their input.
                # Preserve pristine operands before execution; cloning them
                # afterwards would record silu(x) and validate silu(silu(x)).
                pristine_args = clone_tree(args) if node.name in targets else None
                pristine_kwargs = clone_tree(kwargs) if node.name in targets else None
                result = super(_Recorder, inner).run_node(node)
                if node.name in targets:
                    inner.records[node.name] = {
                        "args": pristine_args,
                        "kwargs": pristine_kwargs,
...
def main() -> int:
    ...
    parser.add_argument("--metrics-replays", type=int, default=1000)
    ...
    for index, case in enumerate(cases, start=1):
        ...
        # Each case is a separate profiler capture range.  Nsight Systems can
        # then sample GPU occupancy only while this isolated group is active.
        ...
        # Validate one replay while the pristine inputs are still available.
        # Repeating an in-place graph (for example YOLO SiLU_) intentionally
        # mutates its static input buffer, so correctness must not be checked
        # only after the 1000th metrics replay.
        graph.replay()
        torch.cuda.synchronize()
        correctness = {
