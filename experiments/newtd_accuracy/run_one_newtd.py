...
def main() -> int:
    ...
    try:
        ...
        newtd_state = None
        newtd_original = None
        use_newtd = os.environ.get("JANUS_NEW_TD_PAIR_EXTENSION") == "1"
        if use_newtd:
            if task.variant != "TD+DRT":
                raise ValueError(
                    "JANUS_NEW_TD_PAIR_EXTENSION requires --variant TD+DRT"
                )
            solo_root_value = os.environ.get("JANUS_NEW_TD_SOLO_ROOT")
            if not solo_root_value:
                raise ValueError("JANUS_NEW_TD_SOLO_ROOT is required")
            # Accuracy experiments keep the historical ``strategy`` selector so
            # that they isolate the New-TD admission rule.  The release branch
            # can combine New-TD with the existing NCU-aware interference
            # selector without changing that default protocol.
            params["final_selector"] = os.environ.get(
                "JANUS_NEW_TD_FINAL_SELECTOR", "strategy"
            )
            if params["final_selector"] not in {
                "strategy",
                "timeline",
                ...
                raise ValueError(
                    "unsupported JANUS_NEW_TD_FINAL_SELECTOR: "
                    f"{params['final_selector']}"
                )
            from Opara.Scheduler import ResourceModel
            from newtd_pair_admission import install_newtd_pair_admission

            newtd_original, newtd_state = install_newtd_pair_admission(
                ResourceModel,
                model=task.model,
                profile_sha256=sha256_file(profile),
                solo_profile_roots=[Path(solo_root_value)],
                launch_gap_ms=float(
        ...
        try:
            ...
            if newtd_original is not None:
                from Opara.Scheduler import ResourceModel
                from newtd_pair_admission import restore_newtd_pair_admission
